...
const BROWSER_DEBUGGING_PORT = process.env.chromeLocationPort || 9222;

const TIMEOUT_STATUS_CODE = process.env.TIMEOUT_STATUS_CODE

const CHROME_PATH = process.env.chromeLocationPath

const CHROME_START_FLAGS = process.env.chromeStartFlags
...
server.init = function(options) {
	...
	this.options.chromeLocation = CHROME_PATH || undefined
	this.options.chromeFlags = CHROME_START_FLAGS && JSON.parse(CHROME_START_FLAGS) || undefined
...
// 用于区分是定时重启 chrome，还是手动主动关闭应用，亦或是启动chrome失败。
// '主动关闭应用'或'启动chrome失败'时会调用该函数，isBrowserClosing 为 true，不会自动重启 chrome。
...
server.onRequest = function(req, res) {
	...
	.then(() => {
		...
		if (req.prerender.javascript) {
			return this.browser.executeJavascript(req.prerender.tab, req.prerender.javascript, req.prerender.sleep);
		} else {
		...
		util.log('req headers :', req.prerender.headers + " ; content : " + req.prerender.content)
...
server._send = function(req, res) {
	...
	if (!req.prerender.prerenderData && req.prerender.content) {
		res.send(header + req.prerender.content);
	}
