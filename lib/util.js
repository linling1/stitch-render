...
const PROXY_OFFLINE_API = "http://spider-offline-goproxy.nb-sandbox.com/proxy/list";
...
util.getOptions = function(req) {
	...
	if(requestedUrl.indexOf('/render') === 0) {
		...
		return {
			...
			mobile : optionsObj.mobile === 'true' || false,
			refresh : optionsObj.mobile === 'true' || false,
		}
...
util.getChromeLocation = function () {
	...
	if (platform === 'linux') {
		return '/root/chrome/linux-126.0.6478.126/chrome-linux64/chrome';
	}
