...
chrome.spawn = function (options) {
	return new Promise((resolve, reject) => {
		...
		// TODO : 新增 this.options.ua_pool 逻辑，在后面可以用上。如果用户没有传 ua，则使用 ua_pool 里的 ua
		// TODO : 新增 this.options.proxy_pool 逻辑，在后面可以用上。如果用户没有传 proxy，则使用 proxy_pool 里的 proxy
	...
	return new Promise((resolve, reject) => {
		...
		connectToBrowser(this.webSocketDebuggerURL, this.options.browserDebuggingPort)
			.then((chromeBrowser) => {
				...
				// TODO : createBrowserContext 可以添加 proxyServer 参数
...
chrome.setUpEvents = async function (tab) {
	...
	if (tab.prerender.disable_image == true) {
	...
	Network.responseReceived((params) => {
		...
		// console.log(`===== Initial response from ${params.response.url} with status code ${params.response.status} ; status text : ${params.response.statusText} ; headers : ${JSON.stringify(params.response.headers)}`);
	...
	return new Promise(async (resolve, reject) => {
		...
		Page.enable()
			.then(() => {
				...
				// TODO : addScriptToEvaluateOnNewDocument 添加取消 async 属性的逻辑
