...
chrome.setUpEvents = async function (tab) {
	...
	Network.requestIntercepted(({interceptionId, request, requestId}) => {
		...
		if (!blocked && tab.prerender.disableImage) {
			blocked = (pathname.endsWith('.png') || pathname.endsWith('.img') || pathname.endsWith('.svg') || pathname.endsWith('.jpeg') || pathname.endsWith('.gif'));
	...
	Network.responseReceived(async (params) => {
		...
		if (tab.prerender.interception.hasOwnProperty("requestId") && tab.prerender.interception.requestId && params.requestId == tab.prerender.interception.requestId) {
			let obj = await Network.getResponseBody({'requestId':params.requestId});
			let {body, base64Encoded} = obj;
			tab.prerender.interception.resp = body;
			console.log(`body : ${body} ; base64Encoded : ${base64Encoded}`);
		}
