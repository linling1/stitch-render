...
chrome.openTab = function (options) {
	return new Promise((resolve, reject) => {
		...
		const connectToBrowser = async (target, port) => {
			...
			for(;;) {
				try {
					let client = await CDP({target, port});
					return client;
				} catch (err) {
		...
		connectToBrowser(this.webSocketDebuggerURL, this.options.browserDebuggingPort)
			.then((chromeBrowser) => {
				...
				/* 	Target.createBrowserContext(disposeOnDetach bool ,proxyServer string ,proxyBypassList string)
						i. proxyServer: Proxy server, similar to the one passed to --proxy-server
							[--proxy-server]: This tells Chrome to use a custom proxy configuration.
											  eg. --proxy-server="127.0.0.1:1087;..."
						ii. proxyBypassList: Proxy bypass list, similar to the one passed to --proxy-bypass-list.
							[--proxy-bypass-list]: This tells chrome to bypass any specified proxy for the given semi-colon-separated list of hosts. This flag must be used (or rather, only has an effect) in tandem with --proxy-server.
Note that trailing-domain matching doesn't require "." separators so "*google.com" will match "igoogle.com" for example.
												   eg. --proxy-bypass-list="*.google.com;*foo.com;127.0.0.1:8080"
					https://www.chromium.org/developers/design-documents/network-settings
				 */
				// return browser.Target.createBrowserContext(false,"127.0.0.1:1087");
				...
				// 支持 width、height 参数。
				// todo : 测试是否一个browser支持多种宽高模式渲染？？？
				// todo : 这里已经有url参数了，为什么不可以填写真实的url了？
...
chrome.executeJavascript = function (tab, javascript, sleep) {
	timeout = sleep > 0 ? sleep : 1000
	return new Promise((resolve, reject) => {
		tab.Runtime.evaluate({
