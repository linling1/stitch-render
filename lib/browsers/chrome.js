...
chrome.setUpEvents = async function (tab) {
	...
	tab.prerender.interception = {};
	if (tab.prerender.interceptionStr) {
		tab.prerender.interception.str = tab.prerender.interceptionStr;
	}
	Network.requestIntercepted(({interceptionId, request, requestId}) => {
        // perform a test against the intercepted request
		if (tab.prerender.interception.hasOwnProperty('str') && tab.prerender.interception.str && request.url.includes(tab.prerender.interception.str)) {
			tab.prerender.interception.requestId = requestId;
			tab.prerender.interception.url = request.url;
		}
	...
	Network.requestWillBeSent((params) => {
		...
		if (tab.prerender.logRequests || this.options.logRequests) {
			// cookies = await Runtime.evaluate({
			...
			console.log(`+ numRequestsInFlight : ${tab.prerender.numRequestsInFlight} ; url : ${params.request.url} ; headers : ${JSON.stringify(params.request.headers)} ; cookie : ${JSON.stringify(cookies)}`)
		}
	...
	Network.responseReceived((params) => {
		...
		// for (let k in params.response.headers) {
		// 	if (k && k == "x-ms-token") {
		// 		try {
		// 			console.log(`set msToken : ${params.response.headers[k]}`)
		// 			Network.setCookie({'name':'msToken','value':params.response.headers[k], 'domain':'.tiktok.com'});
		// 		} catch (error) {
		// 			console.log(error)
		// 		}
		// 	}
		...
		if (tab.prerender.interception.hasOwnProperty("requestId") && tab.prerender.interception.requestId && params.requestId == tab.prerender.interception.requestId) {
			tab.prerender.interception.url = params.response.url
			tab.prerender.interception.statusCode = params.response.status
			tab.prerender.interception.headers = params.response.headers
		}
	...
	return new Promise((resolve, reject) => {
		...
		Page.enable()
			.then(() => {
				...
				Page.addScriptToEvaluateOnNewDocument({source:stealth_content})
				// .then(() => {
				// 	if (tab.prerender.disableScriptAsync) {
				// 		return Page.addScriptToEvaluateOnNewDocument({source: `sarr = document.getElementsByTagName('script'); for (let i=0; i < sarr.length; i++){sarr[i].removeAttribute('nonce'); sarr[i].removeAttribute('async');}`, runImmediately:true});
				// 	}
				// })
